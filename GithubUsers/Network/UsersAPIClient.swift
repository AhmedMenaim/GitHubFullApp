...
class UsersAPIClient {
  ...
  func getUserDetails(
    username: String,
    completion: @escaping (Result<UserDetailsNetworkResponse, SessionDataTaskError>) -> Void
  ) {
    ...
    session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
      ...
      do {
        let decoder = JSONDecoder()
        let response = try decoder.decode(UserDetailsNetworkResponse.self, from: data)
        debugPrint(response)
        completion(.success(response))
